...
func _save_local(animal_id: String):
	var path = "user://progress_%s.json" % animal_id
...
# Usado en modo invitado y como fallback cuando Supabase no está disponible.
func load_local(animal_id: String):
	_apply_local_file(animal_id)
	_derive_unlocked_from_done(animal_id)   # por si el archivo es de versión antigua
...
func load_supabase(animal_id: String):
	...
	if rows.is_empty():
		...
		var local_path = "user://progress_%s.json" % animal_id
